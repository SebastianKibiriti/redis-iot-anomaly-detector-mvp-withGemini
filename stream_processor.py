...
# --- NEW: Anomaly Detection Thresholds and Stream ---
MIN_TEMP = 18.0 # Minimum acceptable temperature in Celsius
MAX_TEMP = 28.0 # Maximum acceptable temperature in Celsius
            ...
            for stream_name, stream_messages in messages:
                for message_id, message_data_bytes in stream_messages:
                    try:
                        ...
                        # --- NEW: Publish Alert to a new Stream if an Anomaly is Found ---
                        if alert_data:
                            # Use XADD to add the alert to a new stream
                            r.xadd(ANOMALY_ALERTS_STREAM, alert_data)
                            print(f"*** ANOMALY DETECTED! *** Published alert to '{ANOMALY_ALERTS_STREAM}': {alert_data}")

                        # Store the data in the Time Series (regardless of anomaly status)
